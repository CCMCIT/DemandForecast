"""Unit test (no DB): the OutOfService writer resolves each name under its FieldType, puts
...
from app.processing.out_of_service.writer import (
...
class FakeDetailsRepo:
    """Stands in for OutOfServiceUnitsDetailRepository: serves preset existing rows for
    get_by_dates and captures inserts from add_all."""

    def __init__(self, existing: list[OutOfServiceUnitsDetail] | None = None):
        self.existing = existing or []
        self.added: list[OutOfServiceUnitsDetail] = []
...
def _mapped(**overrides) -> MappedOutOfService:
    base = dict(
    ...
    return MappedOutOfService(**base)
...
def _write(mapped, existing=None, ids=RESOLVED_IDS):
    ...
    writer = OutOfServiceWriter(session=None, resolver=FakeResolver(ids), details=repo)
    writer.write_details(mapped)
    return repo


def _existing_row(**overrides) -> OutOfServiceUnitsDetail:
    """A target row already in the DB, with the SAME identity as the default _mapped()."""
    base = dict(
        OutOfServiceUnitsDetailId=555, LoadId=1, Date=date(2026, 1, 14),
        FieldTypeValueEquipTypeId=202, FieldTypeValueLocationId=404,
        Units=99,
    )
    base.update(overrides)
    return OutOfServiceUnitsDetail(**base)
...
def test_existing_identity_updates_in_place_not_inserted():
    ...
    assert existing.OutOfServiceUnitsDetailId == 555  # same row, id untouched
