"""Purpose: upsert MappedOnTerm rows into OnTermDetail_tbl.

Each of the two descriptive names (Equipment / Location) is resolved to a FieldTypeValue
id under its FieldType, and the id lands in the matching column. Resolution goes through
the DB proc GateActivityFieldTypeValue_upsert (a generic find-or-create; the gate-activity
name is historical), so a value new to the dimension is created, not rejected -- the sets
are open.
...
same OnTermDetailId, temporal archives the prior version -- and only its Units/LoadId change.
A new identity is inserted. Within one batch a repeated identity is collapsed last-wins.
...
from app.db.repositories.on_term_detail_repository import OnTermDetailRepository
from app.lookups import FieldType
from app.processing.on_terminal.dto import MappedOnTerm

# (MappedOnTerm attr holding the name, FieldType to resolve under, target column)
ON_TERM_FIELD_MAP = [
    ("equip_code",    FieldType.EQUIPMENT_TYPE, "FieldTypeValueEquipTypeId"),
    ("location_name", FieldType.LOCATION,       "FieldTypeValueLocationId"),
]

# The columns that identify a row (Date + the two dimensions). Two rows with the same
# values here are the same on-terminal measurement; everything else is mutable payload.
IDENTITY_COLUMNS = [
    "Date",
...
class FieldTypeValueResolver:
    ...
    Duplicated from the gate-activity writer on purpose: on_terminal is its own domain
...
def _identity(row) -> tuple:
    """The 3-column identity of an OnTermDetail (existing row or freshly built)."""
...
class OnTermWriter:
    def __init__(self, session, resolver: FieldTypeValueResolver | None = None,
                 details: OnTermDetailRepository | None = None):
        self.details = details or OnTermDetailRepository(session)
        self.resolver = resolver or FieldTypeValueResolver(session)

    def write_details(self, mapped: list[MappedOnTerm]) -> None:
        ...
        incoming: dict[tuple, OnTermDetail] = {}
        for m in mapped:
    ...
    def _to_detail(self, m: MappedOnTerm) -> OnTermDetail:
        resolved_ids = {
            column: self.resolver.resolve(int(field_type), getattr(m, attr))
            for attr, field_type, column in ON_TERM_FIELD_MAP
        }
        return OnTermDetail(
            LoadId=m.file_id,
            Date=m.date,
