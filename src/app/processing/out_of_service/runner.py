"""Purpose: process CMS out-of-service files into OutOfServiceUnitsDetail_tbl.

Driven through Load_tbl, like the on-terminal runner: a file carries its LoadTypeId and
LoadStatusId, and processing advances the status. Out of service has no phases -- it reads
the file's staging rows, upserts each by its identity (Date + the two dimension columns), and
...
- process_file(file_id): process/reprocess one out-of-service file.
- process_pending(): process every out-of-service file at status 2.
"""
from app.lookups import FileType, LoadStatus
from app.db.session import SessionLocal, session_scope
from app.db.repositories.load_repository import LoadRepository
from app.db.repositories.cms_out_of_service_units_detail_repository import (
    CmsOutOfServiceUnitsDetailRepository,
)
from app.db.repositories.process_log_error_repository import ProcessLogErrorRepository
from app.processing.out_of_service.mapper import map_row
from app.processing.out_of_service.writer import OutOfServiceWriter


def process_file(file_id: int) -> int:
    """Map an out-of-service file's staging rows into OutOfServiceUnitsDetail_tbl, advance the
    ...
    try:
        ...
        if file.LoadTypeId != FileType.OUT_OF_SERVICE:
            raise ValueError(
                f"LoadId {file_id} is not an Out of Service file "
                f"(LoadTypeId={file.LoadTypeId})"
            )

        rows = CmsOutOfServiceUnitsDetailRepository(session).get_by_file_id(file_id)
        mapped = [map_row(row) for row in rows]

        OutOfServiceWriter(session).write_details(mapped)
        # Advance 2 -> 4 directly; out of service has no phase 3, so nothing sets 3.
        # Status 4 is reused as the terminal "done" marker for now, matching on terminal;
        ...
        ProcessLogErrorRepository.write(
            f"Out of service process failed for LoadId {file_id}: {exc}"
        )
        raise
    finally:
        session.close()
...
def process_pending(progress=None) -> dict:
    """Process every out-of-service file at INSERTED_INTO_FILE_DETAIL (2). One file's failure
    ...
    with session_scope() as session:
        files = LoadRepository(session).get_by_type_and_status(
            FileType.OUT_OF_SERVICE, LoadStatus.INSERTED_INTO_FILE_DETAIL
        )
